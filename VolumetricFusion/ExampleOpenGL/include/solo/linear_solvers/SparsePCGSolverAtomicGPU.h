...
namespace solo {
	...
	template<typename FloatType>
	FloatType solve_SparsePCGSolverAtomicGPU(
		vector<std::pair<ResidualVector<FloatType>, JacobianMatrix<FloatType>>>& systemComponents,
		DenseMatrix<int>& columnIndexVector,
		DenseMatrix<int>& rowIndexVector,
		unsigned nParameters,
	...
	template<>
	inline float solve_SparsePCGSolverAtomicGPU<float>(
		...
		return solveImpl_SparsePCGSolverAtomicGPU(
	...
	template<>
	inline double solve_SparsePCGSolverAtomicGPU<double>(
	 ...
	 */
	template<typename FloatType>
	class SparsePCGSolverAtomicGPU : public LinearSolver<FloatType> {
	public:
		SparsePCGSolverAtomicGPU(const Settings& settings) :
		...
		FloatType solve(
			...
			#ifdef COMPILE_CUDA
			return solve_SparsePCGSolverAtomicGPU(
			...
			#else
			throw std::runtime_error("SparsePCGSolverAtomicGPU is not supported without a COMPILE_CUDA preprocessor flag.");
			#endif
		}

		LinearSolverType getType() const override {
			return LinearSolverType::SPARSE_PCG_ATOMIC_GPU;
		}
