...
namespace solo {
	...
	/**
	 * General implemention of SparsePCGSolverCompleteGPU.
	 */
	...
	FloatType solveImpl_SparsePCGSolverCompleteGPU(
		...
		TIME_GPU_START(PCGSolver_ConstructJTJ);
		...
		system_proc::constructJTJ(
			...
			nParameters, jacobianValues.getSize(), residualValues.getSize()
		);

		TIME_GPU_STOP(PCGSolver_ConstructJTJ);
		...
		nIterations = system_proc::executePCGAlgorithm(
			...
			system_proc::JTJApplicationType::COMPLETE
		);
		...
		// TODO: Check early stop termination criteria.
	 ...
	 */
	template float solveImpl_SparsePCGSolverCompleteGPU<float>(
	...
	template double solveImpl_SparsePCGSolverCompleteGPU<double>(
