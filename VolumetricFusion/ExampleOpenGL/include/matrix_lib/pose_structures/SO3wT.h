...
namespace matrix_lib {
	 ...
	 */
	template<typename T>
	class PoseIncrement<T, PoseType::SO3wT> {
	public:
		enum { Type = PoseType::SO3wT };
		...
		template <class U>
		CPU_AND_GPU PoseIncrement(const PoseIncrement<U, PoseType::SO3wT>& other) :
		...
		CPU_AND_GPU static PoseIncrement<T, PoseType::SO3wT> interpolate(const vector<PoseIncrement<T, PoseType::SO3wT>>& poses, const vector<T>& weights) {
			runtime_assert(poses.size() == weights.size(), "Number of poses should equal the number of weights.");
			PoseIncrement<T, PoseType::SO3wT> interpolatedPose{ Vec3<T>{ T(0) }, Vec3<T>{ T(0) } };
		...
		CPU_AND_GPU static PoseIncrement<T, PoseType::SO3wT> identity() { return PoseIncrement<T, PoseType::SO3wT>{}; }
		CPU_AND_GPU static PoseIncrement<T, PoseType::SO3wT> translation(const Vec3<T>& translation) { return PoseIncrement<T, PoseType::SO3wT>{ Mat3<T>::identity(), translation }; }
		CPU_AND_GPU static PoseIncrement<T, PoseType::SO3wT> rotation(const Mat3<T>& rotation) { return PoseIncrement<T, PoseType::SO3wT>{ rotation, Vec3<T>{} }; }
		CPU_AND_GPU static PoseIncrement<T, PoseType::SO3wT> pose(const Mat4<T>& pose) { return PoseIncrement<T, PoseType::SO3wT>{ pose }; }
		CPU_AND_GPU static PoseIncrement<T, PoseType::SO3wT> pose(const Mat3<T>& rotation, const Vec3<T>& translation) { return PoseIncrement<T, PoseType::SO3wT>{ rotation, translation }; }

		/**
		 * Stream output.
		 */
		CPU_AND_GPU friend std::ostream& operator<<(std::ostream& os, const PoseIncrement<T, PoseType::SO3wT>& obj) {
