...
namespace matrix_lib {
	...
	public:
		 ...
		 */
		CPU_AND_GPU SO3(const Vec3<T>& omega) : m_omega{ omega } { }
		CPU_AND_GPU SO3(const T x, const T y, const T z) : SO3(Vec3<T>{ x, y, z }) { }

		/**
		 * Constructor from 3x3 rotation matrix (needs conversion).
		 */
		CPU_AND_GPU SO3(const Mat3<T>& matrix) {
		 ...
		 */
		CPU_AND_GPU SO3(const float4& v) : m_omega{ v }  { }
		...
		template <class U>
		CPU_AND_GPU SO3(const SO3<U>& other) : m_omega{ other.m_omega } { }
		 ...
		 */
		CPU_AND_GPU const SO3<T>& operator=(T value) {
			m_omega = value;
			return *this;
		}
		 ...
		 */
		CPU_AND_GPU UnitQuaternion<T> toQuaternion() const;

		/**
		 * Interpolates the given SO3 rotations with given interpolation weights.
		 * The interpolation weights need to sum up to 1.
		 */
		CPU_AND_GPU static SO3<T> interpolate(const vector<SO3<T>>& rotations, const vector<T>& weights) {
			runtime_assert(rotations.size() == weights.size(), "Number of quaternions should equal the number of weights.");
			Vec3<T> interpolatedVector{ T(0) };
			const unsigned nRotations = rotations.size();

			for (int i = 0; i < nRotations; ++i) {
				const auto& rotation = rotations[i];
				const T& weight = weights[i];
				interpolatedVector += weight * rotation.m_omega;
			}

			return SO3{ interpolatedVector };
		}

		/**
		 * Getters.
		 */
		CPU_AND_GPU const Vec3<T>& getOmega() const { return m_omega; }
		CPU_AND_GPU Vec3<T>& getOmega() { return m_omega; }
		 ...
		 * Stream output.
		 */
		CPU_AND_GPU friend std::ostream& operator<<(std::ostream& os, const SO3<T>& obj) {
	...
	template <typename T>
	CPU_AND_GPU UnitQuaternion<T> SO3<T>::toQuaternion() const {
		const T theta2 = m_omega | m_omega;
