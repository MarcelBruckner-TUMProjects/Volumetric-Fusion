...
namespace matrix_lib {
	...
	public:
		 ...
		 */
		CPU_AND_GPU Pose(const UnitQuaternion<T>& rotation, const Vec3<T>& translation) :
			m_rotation{ rotation },
		 ...
		 */
		CPU_AND_GPU Pose(const Mat3<T>& rotation, const Vec3<T>& translation) :
			m_rotation{ rotation },
			m_translation{ translation }
		{ }

		CPU_AND_GPU Pose(const Mat4<T>& pose) : Pose(pose.getRotation(), pose.getTranslation()) { }
		 ...
		 */
		CPU_AND_GPU Pose(const Vec3<T>& rotation, const Vec3<T>& translation) :
			m_rotation{ SO3<T>(rotation).toQuaternion() },
			m_translation{ translation } 
		{ }

		CPU_AND_GPU Pose(const Vec6<T>& pose) : Pose(Vec3<T>{ pose[0], pose[1], pose[2] }, Vec3<T>{ pose[3], pose[4], pose[5] }) { }

		/**
		 * Initialize with float4.
		 */
		CPU_AND_GPU Pose(const float4& rotation, const float4& translation) :
			m_rotation{ rotation },
			m_translation{ translation }
		{ }
		...
		template <class U>
		CPU_AND_GPU Pose(const Pose<U, PoseType::QUATERNION>& other) :
		...
		CPU_AND_GPU void update(const Pose<T, PoseType::QUATERNION>& pose) {
			m_translation = pose.getTranslation();
			m_rotation = pose.getQuaternion();
		}

		CPU_AND_GPU static Pose<T, PoseType::QUATERNION> identity() {
			return Pose<T, PoseType::QUATERNION>{};
		}

		CPU_AND_GPU static Pose<T, PoseType::QUATERNION> translation(const Vec3<T>& translation) {
			return Pose<T, PoseType::QUATERNION>{ UnitQuaternion<T>{ 1, 0, 0, 0 }, translation };
		}

		CPU_AND_GPU static Pose<T, PoseType::QUATERNION> rotation(const Mat3<T>& rotation) {
			return Pose<T, PoseType::QUATERNION>{ rotation, Vec3<T>{ 0, 0, 0 } };
		}

		CPU_AND_GPU static Pose<T, PoseType::QUATERNION> pose(const Mat4<T>& pose) {
			return Pose<T, PoseType::QUATERNION>{ pose };
		}

		CPU_AND_GPU static Pose<T, PoseType::QUATERNION> pose(const Mat3<T>& rotation, const Vec3<T>& translation) {
			return Pose<T, PoseType::QUATERNION >{ rotation, translation };
		}

		CPU_AND_GPU static Pose<T, PoseType::QUATERNION> interpolate(const vector<Pose<T, PoseType::QUATERNION>>& poses, const vector<T>& weights) {
		 ...
		 */
		CPU_AND_GPU Pose& operator*=(const Pose& rhs) {
		 ...
		 * Getters.
		 */
		CPU_AND_GPU const UnitQuaternion<T>& getQuaternion() const { return m_rotation; }
		CPU_AND_GPU UnitQuaternion<T>& getQuaternion() { return m_rotation; }
		 ...
		 * Setters.
		 */
		CPU_AND_GPU void setQuaternion(const UnitQuaternion<T>& quaternion) { m_rotation = quaternion; }
		CPU_AND_GPU void setTranslation(const Vec3<T>& translation) { m_translation = translation; }

		/**
		 * Returns the inverse pose.
		 */
		CPU_AND_GPU Pose<T, PoseType::QUATERNION> getInverse() const {
		 ...
		 */
		CPU_AND_GPU Mat4<T> getVisionProjection(const Mat3<T>& intrinsicMatrix) const {
		 ...
		 */
		CPU_AND_GPU Mat4<T> getGraphicsProjection(const Mat3<T>& intrinsicMatrix, unsigned width, unsigned height, float depthMin, float depthMax) const {
			Mat4<T> projection = Mat4<T>::identity();
		 ...
		 */
		CPU_AND_GPU static Pose<T, PoseType::QUATERNION> rotation(const UnitQuaternion<T>& rotation) {
		 ...
		 * Stream ouput.
		 */
		CPU_AND_GPU friend std::ostream& operator<<(std::ostream& os, const Pose<T, PoseType::QUATERNION>& obj) {
		 ...
		 * Indexing operators.
		 */
		CPU_AND_GPU UnitQuaternion<T>& at(I<0>) { return m_rotation; }
		CPU_AND_GPU const UnitQuaternion<T>& at(I<0>) const { return m_rotation; }
