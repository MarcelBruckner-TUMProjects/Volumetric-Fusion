...
namespace matrix_lib {
	namespace soa {
		 ...
		 * in SoA.
		 */
		CPU_AND_GPU inline Mat3f load(const iAMat3f& soa, unsigned i = 0) {
			return Mat3f{ soa.xrow()[i], soa.yrow()[i], soa.zrow()[i] };
		}
		CPU_AND_GPU inline void store(const Mat3f& m, iAMat3f& soa, unsigned i = 0) {
		...
		CPU_AND_GPU inline Mat4f load(const iAMat4f& soa, unsigned i = 0) {
			return Mat4f{ soa.xrow()[i], soa.yrow()[i], soa.zrow()[i], soa.wrow()[i] };
		}
		CPU_AND_GPU inline void store(const Mat4f& m, iAMat4f& soa, unsigned i = 0) {
		...
		CPU_AND_GPU inline SO3wTf load(const iASO3wTf& soa, unsigned i = 0) {
			return SO3wTf{ soa.rotation()[i], soa.translation()[i] };
		}
		CPU_AND_GPU inline void store(const SO3wTf& p, iASO3wTf& soa, unsigned i = 0) {
		...
		CPU_AND_GPU inline RigidPosef load(const iAQuaternionPosef& soa, unsigned i = 0) {
			return RigidPosef{ soa.rotation()[i], soa.translation()[i] };
		}
		CPU_AND_GPU inline void store(const RigidPosef& p, iAQuaternionPosef& soa, unsigned i = 0) {
			soa.rotation()[i] = make_float4(p.getQuaternion().real(), p.getQuaternion().imag().x(), p.getQuaternion().imag().y(), p.getQuaternion().imag().z());
			soa.translation()[i] = make_float4(p.getTranslation().x(), p.getTranslation().y(), p.getTranslation().z(), 1.f);
		}

		CPU_AND_GPU inline AffinePosef load(const iAAffinePosef& soa, unsigned i = 0) {
			return AffinePosef{ soa.rotation().xrow()[i], soa.rotation().yrow()[i], soa.rotation().zrow()[i], soa.translation()[i] };
		}
		CPU_AND_GPU inline void store(const AffinePosef& p, iAAffinePosef& soa, unsigned i = 0) {
