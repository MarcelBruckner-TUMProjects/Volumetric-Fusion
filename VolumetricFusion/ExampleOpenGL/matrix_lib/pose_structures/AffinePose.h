...
namespace matrix_lib {
	...
	public:
		 ...
		 */
		CPU_AND_GPU Pose(const Mat3<T>& affineMatrix, const Vec3<T>& translation) :
		 ...
		 */
		CPU_AND_GPU Pose(const Mat4<T>& matrix) :
		 ...
		 */
		CPU_AND_GPU Pose(const float4& matrixXRow, const float4& matrixYRow, const float4& matrixZRow, const float4& translation) :
		...
		template <class U>
		CPU_AND_GPU Pose(const Pose<U, PoseType::AFFINE>& other) :
		...
		CPU_AND_GPU void update(const Pose<T, PoseType::AFFINE>& pose) {
			m_affineMatrix = pose.getAffineMatrix();
			m_translation = pose.getTranslation();
		}

		CPU_AND_GPU static Pose<T, PoseType::AFFINE> identity() {
			return Pose<T, PoseType::AFFINE>{};
		}

		CPU_AND_GPU static Pose<T, PoseType::AFFINE> translation(const Vec3<T>& translation) {
			return Pose<T, PoseType::AFFINE>{ Mat3<T>::identity(), translation };
		}

		CPU_AND_GPU static Pose<T, PoseType::AFFINE> rotation(const Mat3<T>& rotation) {
			return Pose<T, PoseType::AFFINE>{ rotation, Vec3<T>{ 0, 0, 0 } };
		}

		CPU_AND_GPU static Pose<T, PoseType::AFFINE> pose(const Mat4<T>& pose) {
			return Pose<T, PoseType::AFFINE>{ pose };
		}

		CPU_AND_GPU static Pose<T, PoseType::AFFINE> pose(const Mat3<T>& rotation, const Vec3<T>& translation) {
			return Pose<T, PoseType::AFFINE >{ rotation, translation };
		}

		CPU_AND_GPU static Pose<T, PoseType::AFFINE> interpolate(const vector<Pose<T, PoseType::AFFINE>>& poses, const vector<T>& weights) {
		 ...
		 * Stream ouput.
		 */
		CPU_AND_GPU  friend std::ostream& operator<<(std::ostream& os, const Pose<T, PoseType::AFFINE>& obj) {
