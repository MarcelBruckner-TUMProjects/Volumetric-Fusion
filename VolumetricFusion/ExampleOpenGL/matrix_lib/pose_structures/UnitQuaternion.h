...
namespace matrix_lib {	
	...
	public:
		 ...
		 */
		CPU_AND_GPU UnitQuaternion() : m_real{ T(1) }, m_imag{ T(0) } {}

		/**
		 * Explicit constructor.
		 */
		CPU_AND_GPU UnitQuaternion(T r, T i, T j, T k) {
			m_real = r;
			m_imag = Vec3<T>(i, j, k);
			normalize();
		}

		CPU_AND_GPU UnitQuaternion(T real, const Vec3<T>& imag) : m_real(real), m_imag(imag) {
			normalize();
		}
		 ...
		 */
		CPU_AND_GPU UnitQuaternion(const Mat3<T>& matrix) {
		 ...
		 */
		CPU_AND_GPU UnitQuaternion(const float4& v) :
			m_real{ v.x },
			m_imag{ v.y, v.z, v.w }
		{ }
		...
		template <class U>
		CPU_AND_GPU UnitQuaternion(const UnitQuaternion<U>& other) :
			m_real{ other.m_real },
			m_imag{ other.m_imag } 
		{ }
		 ...
		 * Getters.
		 */
		CPU_AND_GPU T real() const { return m_real; }
		CPU_AND_GPU T& real() { return m_real; }
		CPU_AND_GPU const Vec3<T>& imag() const { return m_imag; }
		CPU_AND_GPU Vec3<T>& imag() { return m_imag; }
		 ...
		 */
		CPU_AND_GPU UnitQuaternion<T> operator*(const UnitQuaternion<T>& other) const {
		...
		CPU_AND_GPU UnitQuaternion<T> operator*=(const UnitQuaternion<T>& other) {
			(*this) = (*this) * other;
			return *this;
		}
		 ...
		 */
		CPU_AND_GPU Vec3<T> apply(const Vec3<T>& point) const {
			// p = 0 + px * i + py * j + pz * k
		...
		CPU_AND_GPU Vec3<T> operator*(const Vec3<T>& point) const {
			return apply(point);
		}
		 ...
		 */
		CPU_AND_GPU UnitQuaternion getInverse() const {
		 ...
		 */
		CPU_AND_GPU Mat3<T> matrix() const {
		...
		CPU_AND_GPU SO3<T> toAxisAngle() const;

		/**
		 * Interpolates the given unit quaternions with given interpolation weights.
		 * The interpolation weights need to sum up to 1.
		 */
		CPU_AND_GPU static UnitQuaternion<T> interpolate(const vector<UnitQuaternion<T>>& quaternions, const vector<T>& weights) {
		 ...
		 * Indexing operators.
		 */
		CPU_AND_GPU T& operator[](I<0>) { return m_real; }
		CPU_AND_GPU const T& operator[](I<0>) const { return m_real; }

		CPU_AND_GPU T& operator[](I<1>) { return m_imag[I<0>()]; }
		CPU_AND_GPU const T& operator[](I<1>) const { return m_imag[I<0>()]; }

		CPU_AND_GPU T& operator[](I<2>) { return m_imag[I<1>()]; }
		CPU_AND_GPU const T& operator[](I<2>) const { return m_imag[I<1>()]; }

		CPU_AND_GPU T& operator[](I<3>) { return m_imag[I<2>()]; }
		CPU_AND_GPU const T& operator[](I<3>) const { return m_imag[I<2>()]; }
	...
	private:
		 ...
		 */
		CPU_AND_GPU T squaredLength() const { return m_real * m_real + m_imag[0] * m_imag[0] + m_imag[1] * m_imag[1] + m_imag[2] * m_imag[2]; }
		CPU_AND_GPU T length() const { return sqrt(squaredLength()); }
		 ...
		 */
		CPU_AND_GPU UnitQuaternion<T> getConjugate() const {
		 ...
		 */
		CPU_AND_GPU void constructFromMatrix(const Mat3<T>& m) {
		 ...
		 */
		CPU_AND_GPU friend std::istream& operator>> (std::istream& s, UnitQuaternion<T>& q);
	};
	 ...
	 */
	template<typename T>
	CPU_AND_GPU std::istream& operator>> (std::istream& s, UnitQuaternion<T>& q) { return (s >> q.m_real >> q.m_imag); }
	
	template<typename T>
	CPU_AND_GPU std::ostream& operator<< (std::ostream& s, const UnitQuaternion<T>& q) { return (s << q.real() << " " << q.imag()); }
	 ...
	 */
	template <typename T>
	CPU_AND_GPU SO3<T> UnitQuaternion<T>::toAxisAngle() const  {
		const T& q1 = m_imag.x();
		const T& q2 = m_imag.y();
		const T& q3 = m_imag.z();
